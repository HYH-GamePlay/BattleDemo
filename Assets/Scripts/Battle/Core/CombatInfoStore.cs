...
namespace Battle.Core
{
    ...
    {
        private readonly Dictionary<InfoHandle, ICombatInfo> _infos = new Dictionary<InfoHandle, ICombatInfo>();
        private readonly Dictionary<ICombatInfo, InfoHandle> _handles = new Dictionary<ICombatInfo, InfoHandle>();
        ...
        public InfoHandle GetHandle(ICombatInfo info)
        {
            if (info != null && _handles.TryGetValue(info, out var handle))
            {
                return handle;
        ...
        public void MarkDirty(ICombatInfo info)
        {
